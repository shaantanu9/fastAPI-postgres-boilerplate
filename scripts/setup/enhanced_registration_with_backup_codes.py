...
class EnhancedSecurityCodesService:
    ...
    async def store_backup_codes_in_database(
...
async def backup_code_login(
    request: BackupCodeLoginRequest, db: AsyncSession,
) -> dict[str, Any]:
    ...
    is_valid = await security_service.verify_backup_code_for_login(
        user.id, request.backup_code,
    )
    if not is_valid:
        raise HTTPException(
...
async def backup_code_password_reset(
    request: BackupCodePasswordReset, db: AsyncSession,
) -> dict[str, Any]:
    ...
    is_valid = await security_service.verify_backup_code_for_password_reset(
        user.id, request.backup_code,
    )
    if not is_valid:
        raise HTTPException(
    ...
    await db.execute(
        ...
        .values(
            hashed_password=new_password_hash, password_changed_at=datetime.utcnow(),
        ),
    )
    await db.commit()
...
async def test_enhanced_backup_codes() -> bool | None:
    ...
    try:
        ...
        test_code = codes[0]
        await security_service.verify_backup_code_for_login(
            user_id, test_code,
        )

        # Test password reset verification
        reset_code = codes[1]
        await security_service.verify_backup_code_for_password_reset(
            user_id, reset_code,
        )

        # Check remaining codes
        await security_service.get_remaining_backup_codes_count(user_id)
