    ...
    """Schema for updating {pascal_name} (all fields optional)"""
{
        chr(10).join(
            [
                f"    {field['name']}: Optional[{field['python_type']}] = None"
                for field in fields
            ]
        )
    }
...
def list_plugins() -> None:
    ...
    for plugin_file in plugin_files:
        ...
        "".join(word.capitalize() for word in plugin_name.split("_"))
        ...
        try:
            ...
            ", ".join(features) if features else "Basic CRUD"
...
def health_check() -> None:
    ...
    required_dirs = ["app/plugins", "app/core", "app/db", "app/services"]
...
def fix_plugins() -> None:
    ...
    for plugin_file in plugin_files:
        ...
        try:
            ...
            if (
                "import procrastinate" in content
                and "from app.utils.procrastinate_manager import procrastinate_app"
                not in content
            ):
                ...
                content = content.replace(
                    "@procrastinate.task(", "@procrastinate_app.task(",
                )
                fixed = True
...
Field Constraints:
  max_length=N, min_length=N, ge=N, le=N, gt=N, lt=N, unique, indexed, default=VALUE
    ...
    # List command
    subparsers.add_parser("list", help="List all generated plugins")
    ...
    # Fix plugins command
    subparsers.add_parser(
        "fix-plugins", help="Fix common issues in existing plugins",
    )
    ...
    try:
        if args.command == "add":
            ...
            # Create plugin
            create_plugin_file(
                args.model, fields, args.with_tasks, args.with_bulk,
            )
