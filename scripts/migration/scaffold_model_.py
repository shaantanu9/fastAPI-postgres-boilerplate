...
console = Console()
app = typer.Typer(
    help="FastAPI Project Scaffold Tool (Typer CLI, Rich output, PyYAML, .env, subprocess, admin/user stubs)",
)
...
def prompt_fields():
    fields = []
    console.print(
        "[bold yellow]Enter fields for your model (format: name:type), e.g., title:str. Type 'done' when finished.[/bold yellow]",
    )
    while True:
        field = input("Field: ")
        if field.lower() == "done":
...
def validate_fields(fields):
    ...
    for name, typ in fields:
        if typ not in SUPPORTED_FIELD_TYPES and not typ.startswith("FK:"):
            console.print(
                f"[red]Warning: Field '{name}' has unsupported type '{typ}'. Defaulting to String.[/red]",
            )
            valid = False
    return valid


def edit_base_file_model(model) -> None:
...
def clean_base_file_model(model) -> None:
    path = f"{BASE_PATH}/db/base.py"
    import_statement = f"from app.db.models.{model} import {model.capitalize()}\n"
...
def prompt_relationships():
    rels = []
    console.print(
        "[yellow]Enter relationships (format: field:FK:TargetModel), e.g., author_id:FK:User. Type 'done' when finished.[/yellow]",
    )
    while True:
...
def add_model(
    ...
    if model in track:
        console.print(
            f"[red]Model {model} already scaffolded. Remove first if you want to re-create.[/red]",
        )
        return
    files = {}
    modified_inits = []
    ...
    try:
        ...
        if not dry_run:
            ...
            for subdir, imp in [
                (
                    ...
                    f"from .{snake_case(model)} import {model.capitalize()}\n",
                ),
                (
                    f"{BASE_PATH}/db/schemas/__init__.py",
                    f"from .{snake_case(model)} import {model.capitalize()}Create, {model.capitalize()}Read\n",
                ),
                (
                    f"{BASE_PATH}/services/__init__.py",
                    f"from .{snake_case(model)}_service import {model.capitalize()}Service\n",
                ),
                (
            ...
            save_track(track)
            console.print(
                f"[bold green]Model {model} scaffolded successfully.[/bold green]",
            )
    except Exception as e:
        # Rollback: remove created files, revert __init__.py, revert tracking
        console.print(
            f"[bold red]Error during scaffolding: {e}. Rolling back changes...[/bold red]",
        )
        for path in files:
            if os.path.exists(path):
        ...
        if model in track:
            ...
            save_track(track)
        console.print(
            f"[yellow]Rollback complete. No partial files remain for {model}.[/yellow]",
        )
        raise
    else:
        if dry_run:
            console.print(
                f"[yellow][Dry Run] Would create: {list(files.keys())}[/yellow]",
            )
...
def remove_model(model) -> None:
    ...
    try:
        ...
        for subdir, imp in [
            (
                f"{BASE_PATH}/db/models/__init__.py",
                f"from .{snake_case(model)} import {pascal_case(model)}\n",
            ),
            (
                f"{BASE_PATH}/db/schemas/__init__.py",
                f"from .{snake_case(model)} import {pascal_case(model)}Create, {pascal_case(model)}Read\n",
            ),
            (
                f"{BASE_PATH}/services/__init__.py",
                f"from .{snake_case(model)}_service import {pascal_case(model)}Service\n",
            ),
            (
                f"{BASE_PATH}/api/v1/endpoints/__init__.py",
                f"from .{snake_case(model)} import router as {snake_case(model)}_router\n",
        ...
        save_track(track)
        console.print(
            f"[bold red]Model {model} and all associated files/imports removed.[/bold red]",
        )
    except Exception as e:
        # Rollback: restore files and __init__.py, revert tracking
        console.print(
            f"[bold red]Error during removal: {e}. Rolling back changes...[/bold red]",
        )
        ...
        save_track(track)
        console.print(
            f"[yellow]Rollback complete. Model {model} restored to previous state.[/yellow]",
        )
        raise
...
def add() -> None:
    ...
    if typer.confirm("Run Alembic migration now?", default=False):
        subprocess.run(
            ["alembic", "revision", "--autogenerate", "-m", f"Add {model}"], check=False,
        )
...
def remove() -> None:
    ...
    if typer.confirm("Run Alembic migration now?", default=False):
        subprocess.run(
            ["alembic", "revision", "--autogenerate", "-m", f"Remove {model}"],
            check=False,
        )
        subprocess.run(["alembic", "upgrade", "head"], check=False)
...
@app.command()
def bulk_import(
    yaml_file: str = typer.Argument(..., help="YAML file with models to import"),
) -> None:
    ...
    try:
        for model_spec in spec.get("models", []):
            ...
            examples = model_spec.get(
                "examples", {name: "example" for name, _ in fields},
            )
            add_model(model, fields, relationships, crud, examples)
    except Exception as e:
        console.print(
            f"[bold red]Error during bulk import: {e}. Rolling back all changes...[/bold red]",
        )
        # Restore __init__.py
        ...
        console.print(
            "[yellow]Rollback complete. Project restored to previous state before bulk import.[/yellow]",
        )
        raise
    ...
    class Handler(FileSystemEventHandler):
        def on_modified(self, event) -> None:
            if event.src_path.endswith(".py"):
                console.print(
                    f"[cyan]Detected change in {event.src_path}. Running tests...[/cyan]",
                )
                subprocess.run(["pytest"], check=False)
...
def admin() -> None:
    ...
    if fastapi_admin is None:
        console.print(
            "[yellow]fastapi-admin not installed. Skipping admin scaffolding.[/yellow]",
        )
    else:
...
def user() -> None:
    ...
    if fastapi_users is None:
        console.print(
            "[yellow]fastapi-users not installed. Skipping user scaffolding.[/yellow]",
        )
    else:
...
def update_model(
    model, new_fields, new_relationships, new_crud, new_examples, verbose=True,
) -> None:
    track = load_track()
    if model not in track:
        console.print(
            f"[red]Model {model} not found in scaffolded models. Cannot update.[/red]",
        )
        return
    ...
    try:
        ...
        console.print(
            f"[bold red]Error during update: {e}. Rolling back changes...[/bold red]",
        )
        # Remove new files
        try:
...
def update() -> None:
    ...
    if model not in track:
        ...
        raise typer.Exit
    console.print(
        f"[yellow]Updating {model}. Enter new fields, relationships, CRUD config, and example data. Old files will be replaced.[/yellow]",
    )
    ...
    if typer.confirm("Run Alembic migration now?", default=False):
        subprocess.run(
            ["alembic", "revision", "--autogenerate", "-m", f"Update {model}"],
            check=False,
        )
        subprocess.run(["alembic", "upgrade", "head"], check=False)
    # Optionally run pytest
...
def add_field() -> None:
    ...
    if model not in track:
        console.print(
            f"[red]Model {model} not found in scaffolded models. Cannot add field.[/red]",
        )
        raise typer.Exit
    ...
    new_field_type = typer.prompt(
        "New field type (str/int/float/bool or FK:TargetModel)",
    )
            ...
            for line in f:
                if "crud_ops=" in line:
                    try:
                        ...
                        crud = ast.literal_eval(
                            line.split("crud_ops=")[1].split(")")[0].strip(),
                        )
                    except Exception:
                        pass
    examples = {name: "example" for name, _ in fields}
    ...
    try:
        ...
        console.print(
            f"[bold red]Error during add_field: {e}. Rolling back changes...[/bold red]",
        )
        # Restore __init__.py
        for subdir, content in old_inits.items():
            with open(subdir, "w") as f:
    ...
    if typer.confirm("Run Alembic migration now?", default=True):
        subprocess.run(
            [
                ...
                "--autogenerate",
                "-m",
                f"Add field {new_field_name} to {model}",
            ],
            check=False,
        )
        subprocess.run(["alembic", "upgrade", "head"], check=False)
...
def add_model(model, fields) -> None:
    ...
    try:
        ...
        import_line = (
            f"from app.db.models.{snake_case(model)} import {pascal_case(model)}\n"
        )
...
def remove_model(model) -> None:
    ...
    subprocess.run(
        ["alembic", "revision", "--autogenerate", "-m", f"Remove model {model}"],
        check=False,
    )
    subprocess.run(["alembic", "upgrade", "head"], check=False)
...
""")
    ...
    with open(init_file, "a") as f:
        f.write(
            f"from .{snake_case(model_name)} import {model_name.capitalize()}Create, {model_name.capitalize()}Read\n",
        )
