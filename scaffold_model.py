...
def prompt_enterprise_config():
    ...
    is_tenant_scoped = input("Enable multi-tenancy/organization scoping? (y/n) [n]: ").strip().lower()
    ...
    if rate_limiting:
        ...
        rate_limit_config = {
            "calls_per_minute": int(calls) if calls else 100
        }
    ...
    if enable_caching:
        ...
        cache_config = {
            "ttl_seconds": int(ttl) if ttl else 300
        }
...
def prompt_fields():
    fields = []
    print("\nEnter fields for your model (format: name:type:nullable), e.g., title:str:false. Type 'done' when finished.")
    print("Supported types: str, int, float, bool, datetime, text, json")
    while True:
        field = input("Field: ")
        if field.lower() == "done":
            break
...
def save_scaffold_config(model_name: str, config: Dict[str, Any]):
    """Save scaffold configuration for future reference."""
    if os.path.exists(SCAFFOLD_CONFIG_FILE):
        with open(SCAFFOLD_CONFIG_FILE, 'r') as f:
    ...
    print(f"Configuration saved to {SCAFFOLD_CONFIG_FILE}")
...
def make_model(model_name, fields, config):
    ...
    imports = ["from app.db.base import Base", "from sqlalchemy import Column, String, Integer, Boolean, Float, Text, DateTime, JSON, ForeignKey"]
...
''')
            if config.get("enable_audit"):
                f.write(f'''    creator = relationship("User", foreign_keys=[created_by])
...
''')
        ...
        if config.get("is_tenant_scoped"):
            f.write(f"            'organization_id': self.organization_id,\n")
        
        if config.get("enable_audit"):
            f.write(f'''            'is_active': self.is_active,
...
''')
    ...
    print(f"✅ Created enterprise model: {fname}")
    if config.get("is_tenant_scoped"):
        print(f"  📁 Multi-tenant enabled with organization scoping")
    if config.get("enable_audit"):
        print(f"  📝 Audit logging enabled")

def make_schema(model_name, fields, config):
    ...
    imports = ["from pydantic import BaseModel, Field", "from typing import Optional, Dict, Any"]
...
''')
        ...
        # Add tenant scoping if enabled
        if config.get("is_tenant_scoped"):
            f.write(f"    organization_id: str\n")
        
        # Add audit fields if enabled
        if config.get("enable_audit"):
            f.write(f'''    is_active: bool
    created_at: datetime
    updated_at: datetime
...
''')
    ...
    ensure_init(os.path.dirname(init_file))
    with open(init_file, "a") as f:
        schemas_to_import = [f"{model_name}Create", f"{model_name}Read", f"{model_name}Update", f"{model_name}ListResponse"]
        if config.get("enable_bulk"):
            schemas_to_import.extend([f"{model_name}BulkCreate", f"{model_name}BulkUpdate", f"{model_name}BulkResponse"])
        if config.get("enable_export"):
            schemas_to_import.extend([f"{model_name}ExportRequest", f"{model_name}ExportResponse"])
        
        f.write(f"from .{snake_case(model_name)} import {', '.join(schemas_to_import)}\n")
    
    print(f"✅ Created enterprise schemas: {fname}")
    if config.get("enable_bulk"):
        print(f"  📦 Bulk operations schemas included")
    if config.get("enable_export"):
        print(f"  📤 Export schemas included")

def make_service(model_name, config):
    ...
    if config.get("is_tenant_scoped"):
        imports.append("from app.services.organization_service import organization_service")
