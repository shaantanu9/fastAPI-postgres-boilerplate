...
class AuthenticatedUser(HttpUser):
    ...
    def on_start(self) -> None:
        ...
        with self.client.post(
            f"{API_PREFIX}/auth/register", json=registration_data, catch_response=True,
        ) as response:
    ...
    def login(self) -> None:
        ...
        with self.client.post(
            f"{API_PREFIX}/auth/login", data=login_data, catch_response=True,
        ) as response:
...
class AdminUser(HttpUser):
    ...
    def on_start(self) -> None:
        ...
        admin_data = {"username": "admin@example.com", "password": "AdminPassword123!"}

        with self.client.post(
            f"{API_PREFIX}/auth/login", data=admin_data, catch_response=True,
        ) as response:
            if response.status_code == 200:
                data = response.json()
                self.token = data.get("access_token")
    ...
    @task(3)
    def monitoring_dashboard(self) -> None:
    ...
    @task(2)
    def plugin_management(self) -> None:
    ...
    def system_health(self) -> None:
        ...
        self.client.get(
            f"{API_PREFIX}/plugins/system/health", name="GET /plugins/system/health",
        )
...
class DatabaseStressUser(HttpUser):
    ...
    @task(2)
    def concurrent_writes(self) -> None:
