...
class HealthChecker:
    ...
    def register_check(
        self, name: str, check_func: Callable, is_dependency: bool = False,
    ) -> None:
...
async def check_http_service(url: str, timeout: int = 5) -> HealthCheckResult:
    ...
    try:
        ...
        return HealthCheckResult(
            name=f"http_service_{url}",
            status=HealthStatus.UNHEALTHY,
            message=f"Service {url} is unreachable: {e!s}",
...
def setup_health_checks(app: FastAPI, **kwargs) -> HealthChecker:
    ...
    if "database_engine" in kwargs:
        ...
        health_checker.register_check(
            "database", lambda: check_postgresql_async(engine), is_dependency=True,
        )
    ...
    if "redis_url" in kwargs:
        ...
        health_checker.register_check(
            "redis", lambda: check_redis_async(redis_url), is_dependency=True,
        )
...
if __name__ == "__main__":
    ...
    health_checker = setup_health_checks(
        app, external_services=["https://httpbin.org/get"],
    )
