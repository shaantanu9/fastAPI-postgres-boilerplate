...
class AuthModelsTemplate:
    ...
    def generate(
        ...
        additional_methods = self._generate_additional_methods(
            auth_config, pascal_name, fields,
        )
...
'''
    ...
    def _generate_relationships(self, auth_config: dict[str, Any]) -> str:
        ...
        if auth_config.get("enable_soft_delete", False):
            relationships.append(
                "    deleted_by = relationship('User', foreign_keys=[deleted_by_id], lazy='select')",
            )
    ...
    def _generate_imports(self, auth_config: dict[str, Any]) -> str:
        ...
        if auth_config.get("enable_ownership", False) or auth_config.get(
            "enable_soft_delete", False,
        ):
    ...
    def _generate_dict_fields(
        self, fields: list[dict[str, Any]], auth_config: dict[str, Any],
    ) -> str:
        ...
        if auth_config.get("enable_audit", True):
            dict_fields.extend(
                ["'created_ip': self.created_ip", "'updated_ip': self.updated_ip"],
            )
        ...
        if auth_config.get("enable_versioning", False):
            dict_fields.extend(
                ["'version': self.version", "'revision_notes': self.revision_notes"],
            )
