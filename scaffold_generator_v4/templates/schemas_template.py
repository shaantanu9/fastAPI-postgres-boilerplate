...
class SchemasTemplate:
    ...
    def generate(
        ...
        for field in fields:
            ...
            pydantic_update_fields.append(
                field_validator.get_pydantic_field(field, for_update=True),
            )
    ...
    def _generate_import_statements(self, imports: dict[str, list[str]]) -> str:
        ...
        if pydantic_imports:
            if "EmailStr" in pydantic_imports or "HttpUrl" in pydantic_imports:
                email_url_imports = [
                    imp for imp in pydantic_imports if imp in ["EmailStr", "HttpUrl"]
                ]
                import_lines.append(
                    f"from pydantic import {', '.join(email_url_imports)}",
                )
            ...
            if "datetime" in pydantic_imports or "date" in pydantic_imports:
                datetime_imports = [
                    imp for imp in pydantic_imports if imp in ["datetime", "date"]
                ]
                import_lines.append(
            ...
            if "Dict" in pydantic_imports or "Any" in pydantic_imports:
                [
                    imp for imp in pydantic_imports if imp in ["Dict", "Any"]
                ]
