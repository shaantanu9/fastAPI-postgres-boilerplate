...
'''
    ...
    def _get_operation_timeouts(
        self, fields: list[dict[str, Any]], with_bulk: bool,
    ) -> dict[str, float]:
        """Determine appropriate timeouts based on model complexity."""
        # Analyze field complexity
        has_text_search = any(
            field.get("type") in ["text", "longtext", "json"] for field in fields
        )
        has_relations = any(
            field.get("type") in ["foreignkey", "manytomany"] for field in fields
        )
        ...
        if field_count > 20:
            ...
            timeouts.update(
                {"create": 15.0, "update": 15.0, "search": 25.0, "database": 12.0},
            )

        if has_text_search:
            # Text search operations are slower
            timeouts.update({"search": 30.0, "database": 15.0})

        if has_relations:
            # Related data fetching takes longer
            timeouts.update(
                {"read": 10.0, "create": 15.0, "update": 15.0, "database": 12.0},
            )

        if with_bulk:
            # Bulk operations need much longer timeouts
            timeouts.update(
                {"bulk_create": 60.0, "bulk_update": 60.0, "bulk_delete": 30.0},
            )

        return timeouts

    def _generate_bulk_routes(
        self, pascal_name: str, snake_name: str, timeouts: dict[str, float],
    ) -> str:
