...
class ArchitectureAnalyzer:
    ...
    def _extract_dependencies(self, plugin_path: Path) -> list[str]:
        ...
        return [dep for dep in dependencies if dep.startswith("app")]
    ...
    def _detect_issues(
        self, plugins: list[PluginInfo], dependency_graph: dict[str, list[str]],
    ...
    def _detect_unused_dependencies(
        self, plugins: list[PluginInfo],
    ) -> list[DependencyIssue]:
        ...
        for plugin in plugins:
            if (
                len(plugin.dependencies) > 10
            ):  # Many dependencies might indicate unused ones
                issues.append(
                    DependencyIssue(
                        type="dependencies",
                        source=plugin.name,
    ...
    def _calculate_metrics(
        ...
        return {
            ...
            "dependency_connections": sum(
                len(deps) for deps in dependency_graph.values()
            ),
            "plugins_with_dependencies": len([p for p in plugins if p.dependencies]),
            "largest_plugin_lines": max(
                (p.size_metrics.get("python_lines", 0) for p in plugins), default=0,
            ),
            "most_complex_plugin": max(
                (p.complexity_score for p in plugins), default=0,
            ),
        }
    ...
    def _generate_recommendations(
        ...
        if metrics["total_plugins"] > 10:
            recommendations.append(
                "Consider organizing plugins into categories/modules for better maintainability",
            )

        if metrics["average_complexity"] > 15:
            recommendations.append(
                "Overall code complexity is high - consider refactoring complex functions",
            )

        # Dependency recommendations
        if metrics["dependency_connections"] > metrics["total_plugins"] * 2:
            recommendations.append(
                "High plugin interdependency detected - consider reducing coupling",
            )

        # Size recommendations
        if metrics["largest_plugin_lines"] > 1500:
            recommendations.append(
                "Some plugins are very large - consider splitting into smaller modules",
            )
        ...
        if high_issues:
            recommendations.append(
                "Prioritize fixing high-severity architecture issues",
            )

        # Performance recommendations
        if metrics["total_lines_of_code"] > 10000:
            recommendations.append(
                "Large codebase detected - consider implementing code splitting and lazy loading",
            )

        # Testing recommendations
        recommendations.append(
            "Implement comprehensive integration tests for plugin interactions",
        )
        recommendations.append(
            "Add performance benchmarks for critical plugin operations",
        )

        return recommendations

    def _calculate_health_score(
        self, issues: list[DependencyIssue], metrics: dict[str, Any],
    ) -> float:
    ...
    def generate_report(
        self, report: ArchitectureReport, output_file: Path | None = None,
    ) -> str:
        ...
        output.append(
            f"- **Average Complexity:** {report.metrics['average_complexity']:.1f}",
        )
        output.append("")
        ...
        if report.issues:
            ...
            for severity in ["critical", "high", "medium", "low"]:
                if issue_counts[severity] > 0:
                    output.append(
                        f"- **{severity.title()}:** {issue_counts[severity]} issues",
                    )
            output.append("")
            ...
            for issue in sorted(
                ...
                output.append(
                    f"### {issue.severity.title()}: {issue.type.title()} Issue",
                )
        ...
        for plugin in sorted(
            report.plugins, key=lambda x: x.complexity_score, reverse=True,
        ):
            output.append(f"### {plugin.name}")
            output.append(
                f"- **Models:** {len(plugin.models)} ({', '.join(plugin.models)})",
            )
            output.append(f"- **Routes:** {len(plugin.routes)}")
            output.append(f"- **Dependencies:** {len(plugin.dependencies)}")
            output.append(
                f"- **Lines of Code:** {plugin.size_metrics.get('python_lines', 0)}",
            )
