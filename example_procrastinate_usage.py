...
from app.services.enhanced_procrastinate_service import EnhancedProcrastinateService
...
async def bulk_process_data(
    ...
    if len(data_items) > 10000:
        raise HTTPException(
            status_code=400, detail="Maximum 10,000 items allowed per bulk operation",
        )
    ...
    try:
        job_id = await manager.defer_bulk_processing(
            data_items=data_items, task_type=task_type, priority=priority_enum,
        )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to queue bulk task: {e!s}")
...
async def send_notification_async(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to queue notification: {e!s}",
        )
...
async def smart_bulk_processing(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Smart processing failed: {e!s}")
...
async def schedule_cleanup(
    ...
    try:
        job_id = await manager.schedule_cleanup(days_old=days_old, at=schedule_at)
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to schedule cleanup: {e!s}",
        )
...
async def schedule_user_batch_processing(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to schedule user processing: {e!s}",
        )
...
async def create_direct_task():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Direct task creation failed: {e!s}",
        )
...
async def welcome_user_process(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Welcome process failed: {e!s}")
