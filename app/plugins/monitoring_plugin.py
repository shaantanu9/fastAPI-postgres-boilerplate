...
class MetricsCollector:
    ...
    def _get_top_endpoints(self) -> List[Dict]:
        ...
        return [{"endpoint": endpoint, "count": count} for endpoint, count in sorted_endpoints[:10]]
    ...
    def _perform_health_checks(self) -> Dict:
        ...
        checks["response_time"] = {
            "status": "ok" if avg_response_time < 200 else "warning" if avg_response_time < 500 else "critical",
...
class MonitoringPlugin(PluginBase):
        ...
        async def get_performance():
            ...
            return {
                "response_times": {
                    ...
                    "recent_samples": list(self.metrics_collector.response_times)[-20:]  # Last 20 samples
                },
                "throughput": {
                    "requests_per_minute": metrics["application"]["requests_per_minute"],
    ...
    async def startup(self):
        ...
        self.emit_event("monitoring_ready", features=["metrics", "health_checks", "alerts"])
        ...
        while True:
            try:
                ...
                if error_rate > 10:  # More than 10% error rate
                    self._add_alert("warning", f"High error rate detected: {error_rate:.1f}%")
                
                # Emit periodic health check event
                self.emit_event("periodic_health_check", status=health_status, error_rate=error_rate)
        ...
        while True:
            try:
                ...
                if memory.percent > 90:
                    self._add_alert("critical", f"High memory usage: {memory.percent:.1f}%")
                
                # Check recent errors
                recent_errors = len([e for e in self.metrics_collector.errors 
                                   if datetime.fromisoformat(e["timestamp"]) > datetime.utcnow() - timedelta(minutes=5)])
                if recent_errors > 10:
                    self._add_alert("warning", f"High error count in last 5 minutes: {recent_errors}")
    ...
    def _calculate_error_rate(self) -> float:
        ...
        error_count = sum(count for status, count in self.metrics_collector.status_codes.items() if status >= 400)
    ...
    async def on_user_login(self, **kwargs):
        """Handle user login event"""
        username = kwargs.get("username", "unknown")
        plugin = kwargs.get("plugin", "default")
        print(f"Monitoring: User login detected - {username} via {plugin}")
    
    async def on_user_logout(self, **kwargs):
        """Handle user logout event"""
        plugin = kwargs.get("plugin", "default")
        print(f"Monitoring: User logout detected via {plugin}") 