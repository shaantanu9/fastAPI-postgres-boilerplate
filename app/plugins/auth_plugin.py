...
class AuthPlugin(PluginBase):
    ...
    def setup_routes(self):
        ...
        async def enhanced_login(credentials: dict):
            ...
            if credentials.get("username") == "admin" and credentials.get("password") == "secret":
                ...
                token = jwt.encode(payload, settings.jwt_secret_token, algorithm="HS256")
                
                # Emit login event
                self.emit_event("user_login", username=credentials["username"], plugin="auth_enhanced")
        ...
        async def get_enhanced_user(token: str = Depends(self.security)):
            ...
            try:
                settings = get_settings()
                payload = jwt.decode(token.credentials, settings.jwt_secret_token, algorithms=["HS256"])
    ...
    async def on_startup(self, **kwargs):
        ...
        print("Auth plugin: Application started, initializing authentication backend")
    
    async def on_user_created(self, **kwargs):
        """Handle user creation event"""
        user_id = kwargs.get("user_id")
        print(f"Auth plugin: New user created with ID: {user_id}")
