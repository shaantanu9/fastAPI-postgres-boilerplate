...
from .schemas import TestItemCreate, TestItemResponse, TestItemUpdate
...
class TestItemRoutes:
    ...
    def setup_routes(self) -> None:
        ...
        @self.router.post(
            "/test_items/", response_model=TestItemResponse, tags=["TestItem"],
        )
        ...
        async def create_test_item(item: TestItemCreate, request: Request):
            ...
            try:
                ...
                # Emit creation event
                self.emit_event(
                    "test_item_created", id=result.id, plugin="test_item_plugin",
                )
        ...
        @self.router.get(
            "/test_items/", response_model=list[TestItemResponse], tags=["TestItem"],
        )
        ...
        @self.router.get(
            "/test_items/{item_id}", response_model=TestItemResponse, tags=["TestItem"],
        )
        @with_timeout(timeout_seconds=5.0)
        ...
        async def update_test_item(
            item_id: int, item: TestItemUpdate, request: Request,
        ):
            """Update a test_item."""
            try:
                async with database_timeout_context(15.0):
        ...
        @self.router.post(
            "/test_items/bulk", response_model=list[TestItemResponse], tags=["TestItem"],
        )
        ...
        @with_timeout(timeout_seconds=60.0)
        async def update_bulk_test_items(
            updates: list[TestItemUpdate], request: Request,
        ):
