...
from .schemas import TestItemCreate, TestItemUpdate, TestItemResponse
...
class TestItemService(EnhancedBaseService[TestItem]):
    ...
    async def get_by_name(self, name: str) -> Optional[TestItem]:
        """Get test_item by name if name field exists"""
        if hasattr(TestItem, 'name'):
            return await self.get_by_field("name", name)
        return None
    
    async def search(self, query: str, limit: int = 10, skip: int = 0) -> List[TestItem]:
