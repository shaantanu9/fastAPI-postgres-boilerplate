...
class RateLimitRule:
    def __init__(
        self, limit: int, window: int, rule_type: RateLimitType, path_pattern: str = "*",
    ) -> None:
        self.limit = limit
        self.window = window
...
class AdvancedRateLimiter:
    ...
    def _setup_default_rules(self) -> None:
        ...
        self.rules.append(
            RateLimitRule(5, 300, RateLimitType.PER_IP, "/api/v1/auth/login"),
        )
        self.rules.append(
            RateLimitRule(3, 3600, RateLimitType.PER_IP, "/api/v1/auth/register"),
        )
        self.rules.append(
            RateLimitRule(
                3, 3600, RateLimitType.PER_IP, "/api/v1/user-management/forgot-password",
            ),
        )
    ...
    def _is_suspicious_ip(self, ip: str, request: Request) -> bool:
        ...
        return bool(any(pattern in user_agent for pattern in suspicious_patterns))
