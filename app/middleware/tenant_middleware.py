...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, tenant_header: str = "X-Organization-ID") -> None:
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if tenant_context:
                response.headers["X-Organization-ID"] = tenant_context[
                    "organization_id"
                ]
                response.headers["X-Organization-Slug"] = tenant_context[
                    "organization_slug"
                ]
    ...
    async def _extract_tenant_context(self, request: Request) -> dict | None:
    ...
    def _is_excluded_route(self, path: str) -> bool:
        ...
        return any(re.match(pattern, path) for pattern in self.excluded_routes)

    def _extract_org_slug_from_path(self, path: str) -> str | None:
    ...
    async def _get_user_from_token(self, request: Request) -> str | None:
...
def get_current_organization_slug(request: Request) -> str | None:
