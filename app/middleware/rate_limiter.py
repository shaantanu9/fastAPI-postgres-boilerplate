...
class RateLimitRule:
    ...
    def __init__(
        ...
        country_whitelist: List[str] = None,
        country_blacklist: List[str] = None
    ):
        self.limit = limit
        self.window = window
...
class IPGeolocationService:
    ...
    def get_ip_info(self, ip_address: str) -> Dict[str, str]:
        ...
        if not self.geoip_db:
            return {"country": "unknown", "city": "unknown", "risk_level": RiskLevel.LOW.value}
...
class EnterpriseRateLimiter:
    ...
    def _setup_default_rules(self):
        ...
        self.rules.append(RateLimitRule(
            limit=100, window=3600, rule_type=RateLimitType.PER_IP,
            path_pattern="/api/*", methods=["GET"]
        ))
        
        self.rules.append(RateLimitRule(
            limit=50, window=3600, rule_type=RateLimitType.PER_IP,
            path_pattern="/api/*", methods=["POST", "PUT", "DELETE"]
        ))
        
        # Authentication endpoints - very strict
        self.rules.append(RateLimitRule(
            limit=5, window=300, rule_type=RateLimitType.PER_IP,
            path_pattern="/api/v1/auth/login", methods=["POST"]
        ))
        
        self.rules.append(RateLimitRule(
            limit=3, window=3600, rule_type=RateLimitType.PER_IP,
            path_pattern="/api/v1/auth/register", methods=["POST"]
        ))
        
        # Password reset - strict
        self.rules.append(RateLimitRule(
            limit=3, window=3600, rule_type=RateLimitType.PER_IP,
            path_pattern="/api/v1/user-management/forgot-password", methods=["POST"]
        ))
        
        # Global rate limit per IP
        self.rules.append(RateLimitRule(
            limit=1000, window=3600, rule_type=RateLimitType.PER_IP,
            path_pattern="*"
        ))
    ...
    def _is_country_allowed(self, ip: str, rule: RateLimitRule) -> bool:
        ...
        # Check whitelist
        if rule.country_whitelist and country not in rule.country_whitelist:
            return False
    ...
    async def log_security_event(self, request: Request, event_type: str, details: Dict):
        ...
        logger.warning(f"Security event: {event_type} from {client_ip} ({risk_level.value})")


class RateLimitMiddleware(BaseHTTPMiddleware):
    """FastAPI middleware for rate limiting"""
    
    def __init__(self, app: ASGIApp, rate_limiter: EnterpriseRateLimiter):
        super().__init__(app)
