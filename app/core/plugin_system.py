...
from typing import Dict, List, Optional, Type, Protocol, Any, Callable, Set
...
class PluginRegistry:
    ...
    def _resolve_loading_order(self) -> None:
        ...
        plugins_by_priority = sorted(
            self._plugins.keys(),
            key=lambda name: self._metadata[name].priority
        )
...
class PluginLoader:
    ...
    def _load_from_file(self, file_path: Path) -> Optional[PluginInterface]:
            ...
            for name, obj in inspect.getmembers(module):
                if self._is_plugin_class(obj):
                    ...
                    try:
                        ...
                        logger.error(f"Failed to instantiate plugin class {name}: {instantiation_error}")
                        continue
    ...
    def _is_plugin_class(self, obj: Any) -> bool:
        ...
        if obj.__name__.endswith('Plugin'):
            logger.debug(f"Plugin class check for {obj.__name__}: "
...
class PluginManager:
    ...
    def __init__(self, app: FastAPI, app_version: str = "1.0.0"):
        self.app = app
    ...
    async def discover_and_load_plugins(self, search_paths: List[str]) -> None:
        ...
        for plugin in discovered_plugins:
            try:
                ...
                plugin.metadata.status = PluginStatus.LOADED  # Fixed: Set to LOADED after registration
            ...
            try:
                ...
                if hasattr(plugin, 'get_blueprint'):
                    try:
                        ...
                        if blueprint:
                            from app.utils.procrastinate_manager import register_plugin_blueprint
                            namespace = f"{plugin_name}_tasks"
                            register_plugin_blueprint(blueprint, namespace)
                            logger.info(f"Registered Procrastinate blueprint for plugin: {plugin_name}")
                    except Exception as blueprint_error:
                        logger.warning(f"Failed to register blueprint for plugin {plugin_name}: {blueprint_error}")
                
                logger.info(f"Initialized plugin: {plugin_name} (Status: {plugin.metadata.status.value})")
                
                # Emit plugin initialized event
                self.context.event_bus.emit("plugin_initialized", plugin_name=plugin_name)
        ...
        for plugin_name, plugin in self.registry.get_all_plugins().items():
            if plugin.metadata.status == PluginStatus.INITIALIZED:
                try:
                    ...
                    logger.info(f"Started plugin: {plugin_name} (Status: {plugin.metadata.status.value})")
                except Exception as e:
