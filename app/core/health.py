    ...
    async def _check_system_resources(self) -> HealthCheck:
        ...
        try:
            ...
            if cpu_percent > 90 or load_avg[0] > cpu_count * 2:
                status = HealthStatus.UNHEALTHY
                message = f"High system load: CPU {cpu_percent:.1f}%, Load {load_avg[0]:.2f}"
    ...
    async def run_all_checks(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "healthy": len([c for c in results if c.status == HealthStatus.HEALTHY]),
                "degraded": len([c for c in results if c.status == HealthStatus.DEGRADED]),
                "unhealthy": len([c for c in results if c.status == HealthStatus.UNHEALTHY]),
            }
        }
