...
class RateLimiter:
    ...
    async def _get_client_tier(self, db: AsyncSession, client_id: str) -> RateLimitTier:
    ...
    async def _check_rate_limit(self, redis: Redis, client_id: str, limit: int) -> None:
        ...
        pipeline.zremrangebyscore(key, 0, now - self.override_window)
    ...
    async def _get_api_key_tier(self, db: AsyncSession, api_key: str) -> RateLimitTier:
        ...
        if api_key_data.organization_id:
            return await self._get_organization_tier(db, api_key_data.organization_id)

        # Get user's tier
        return await self._get_user_tier(db, api_key_data.user_id)

    async def _get_user_tier(self, db: AsyncSession, user_id: str) -> RateLimitTier:
        ...
        if org_membership:
            return await self._get_organization_tier(db, org_membership.organization_id)
