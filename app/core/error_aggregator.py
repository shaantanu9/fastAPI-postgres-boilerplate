...
class ErrorGroup:
    ...
    def _calculate_rate(self) -> float:
        ...
        if time_diff < 1:
            time_diff = 1  # Minimum 1 hour for calculation
...
class ErrorAggregator:
    ...
    def _create_fingerprint(self, exception_type: str, message: str, 
                          request_path: str, traceback: str) -> str:
        ...
        fingerprint_data = f"{exception_type}:{message}:{request_path}:{normalized_traceback}"
    ...
    def _update_stats(self, event: ErrorEvent):
        """Update statistics"""
    ...
    def _check_alerts(self, event: ErrorEvent):
        ...
        if event.level in ['CRITICAL', 'ERROR']:
            recent_errors = [
                ...
                if e.level in ['CRITICAL', 'ERROR'] and 
                   (now - e.timestamp).total_seconds() < 60
            ]
            
            if len(recent_errors) > self.stats['alert_thresholds']['critical_errors_per_minute']:
                self._create_alert(
                    'high_error_rate',
        ...
        if group.count == 1:  # First occurrence of this error type
            ...
            if len(recent_new_errors) > self.stats['alert_thresholds']['new_error_types_per_hour']:
                self._create_alert(
                    'new_error_spike',
    ...
    def _create_alert(self, alert_type: str, message: str, severity: str, context: Dict):
        """Create an alert"""
        alert = {
            'id': self._generate_event_id(),
            'type': alert_type,
    ...
    async def _cleanup_old_events(self):
        ...
        # Clean up old groups
        old_groups = [
            fingerprint for fingerprint, group in self.groups.items()
    ...
    def get_dashboard_data(self, hours: int = 24) -> Dict[str, Any]:
        ...
        recent_groups = {
            fp: group for fp, group in self.groups.items()
            if group.last_seen > cutoff_time
        }
        ...
        return {
            'summary': {
                ...
                'affected_paths': len(set(e.request_path for e in recent_events if e.request_path))
            },
    ...
    def get_error_details(self, fingerprint: str) -> Optional[Dict]:
