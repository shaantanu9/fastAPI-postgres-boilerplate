...
class EmailAttachment(BaseModel):
    ...
    @field_validator("content_type", pre=True, always=True)
    @classmethod
    def set_content_type(cls, v, info):
        if v is None and "filename" in info.data:
...
class EmailRequest(BaseModel):
    ...
    @field_validator("tracking_id", pre=True, always=True)
    @classmethod
...
class SESEmailUtility:
    ...
    def __init__(
        ...
        try:
            ...
            logger.error(f"Failed to initialize SES client: {e!s}")
            raise
    ...
    async def send_email(
        ...
        try:
            ...
            logger.error(f"Error in send_email: {e!s}")
    ...
    async def _send_immediate_email(self, email_request: EmailRequest) -> EmailResponse:
        ...
        try:
            ...
            # Prepare recipients
            [
                recipient.email for recipient in email_request.to_recipients
            ]
            (
                [recipient.email for recipient in email_request.cc_recipients]
                if email_request.cc_recipients
                else []
            )
            (
                [recipient.email for recipient in email_request.bcc_recipients]
    ...
    async def _send_simple_email(self, email_request: EmailRequest) -> str:
        ...
        if email_request.configuration_set or self.default_configuration_set:
            email_params["ConfigurationSetName"] = (
                email_request.configuration_set or self.default_configuration_set
            )
    ...
    async def _send_raw_email_with_attachments(
        ...
        # Add configuration set if specified
        if email_request.configuration_set or self.default_configuration_set:
            raw_email_params["ConfigurationSetName"] = (
                email_request.configuration_set or self.default_configuration_set
            )
    ...
    async def _add_attachment(self, msg: MIMEMultipart, attachment: EmailAttachment) -> None:
        """Add attachment to email message."""
        try:
            if (
                attachment.content_type.startswith("image/")
                and attachment.disposition == "inline"
            ):
            ...
            logger.error(f"Error adding attachment {attachment.filename}: {e!s}")
            raise
    ...
    async def _schedule_email(self, email_request: EmailRequest) -> EmailResponse:
        ...
        max(0, (send_time - datetime.utcnow()).total_seconds())
    ...
    async def _validate_email_request(self, email_request: EmailRequest) -> None:
        ...
        if self.from_domain and not email_request.from_email.endswith(
            f"@{self.from_domain}",
        ):
            logger.warning(
                f"Sender email {email_request.from_email} does not match configured domain {self.from_domain}",
            )
    ...
    async def send_template_email(
        ...
        try:
            ...
            if configuration_set or self.default_configuration_set:
                bulk_email_params["ConfigurationSetName"] = (
                    configuration_set or self.default_configuration_set
                )
            ...
            logger.error(f"Error sending template email: {e!s}")
            return EmailResponse(
                success=False,
                tracking_id=str(uuid4()),
                error_message=str(e),
    ...
    async def create_template(
        ...
        try:
            ...
            logger.error(f"Error creating template '{template_name}': {e!s}")
            return False
    ...
    async def verify_domain_identity(self, domain: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error verifying domain {domain}: {e!s}")
            return False
    ...
    async def create_configuration_set(
        ...
        try:
            ...
            logger.error(f"Error creating configuration set '{name}': {e!s}")
            return False
...
async def send_welcome_email(
    ...
    ses_utility: SESEmailUtility | None = None,
) -> EmailResponse:
