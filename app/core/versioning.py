...
class APIVersion:
    ...
    def from_string(cls, version_str: str) -> "APIVersion":
        ...
        return cls(major=int(major), minor=int(minor), patch=int(patch) if patch else 0)
...
class APIVersionManager:
    ...
    def get_version_from_query(self, version: str | None = Query(None)) -> APIVersion:
...
def get_api_version(
    request: Request, strategy: VersioningStrategy = VersioningStrategy.HEADER,
) -> APIVersion:
    ...
    if strategy == VersioningStrategy.QUERY:
...
def create_v1_router() -> VersionedAPIRouter:
    ...
    return VersionedAPIRouter(
        version="1.0.0", prefix="/api/v1", tags=["v1.0"], deprecated=False,
    )
...
def create_v2_router() -> VersionedAPIRouter:
    ...
    return VersionedAPIRouter(
        version="2.0.0", prefix="/api/v2", tags=["v2.0"], deprecated=False,
    )
...
def get_content_version(accept_header: str) -> APIVersion | None:
