...
"""
from typing import Optional, TypeVar, Callable, Awaitable, Any, Type
...
async def timeout_middleware(request: Request, call_next):
    ...
    try:
        ...
        route_timeout = getattr(request.scope.get("endpoint"), "timeout_seconds", DEFAULT_TIMEOUT)
...
@asynccontextmanager
async def database_timeout_context(seconds: float = DATABASE_TIMEOUT) -> AsyncGenerator[None, None]:
    """
    ...
    try:
        ...
        logger.error("Database operation timed out", extra={"timeout_seconds": seconds})
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT,
            detail="Database operation timed out"
        )
    except SQLTimeoutError:
        logger.error("Database query timed out", extra={"timeout_seconds": seconds})
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT,
...
def with_timeout(
    ...
    *,
    ...
    def decorator(f: Callable[..., Awaitable[T]]) -> Callable[..., Awaitable[T]]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            try:
                return await asyncio.wait_for(f(*args, **kwargs), timeout=timeout_seconds)
            except asyncio.TimeoutError:
                logger.warning(
