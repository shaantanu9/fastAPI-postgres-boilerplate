...
class EnterpriseRedisManager:
    ...
    def __init__(self) -> None:
        ...
        self.redis_cluster: Redis | None = None
    ...
    async def _fallback_set(self, key: str, value: Any, ttl: int = 3600) -> None:
    ...
    async def cache_get(
        self, key: str, namespace: RedisNamespace = RedisNamespace.CACHE,
    ) -> Any:
    ...
    async def cache_invalidate_pattern(
        self, pattern: str, namespace: RedisNamespace = RedisNamespace.CACHE,
    ) -> None:
        ...
        try:
            if self.redis_client and self._is_healthy:
                ...
                if keys:
                    ...
                    logger.info(
                        f"Invalidated {len(keys)} cache keys matching {pattern}",
                    )
            else:
                # Fallback pattern matching
                prefix = self._get_key(namespace, "")
                to_delete = [
                    k for k in self._fallback_cache if k.startswith(prefix)
                ]
    ...
    async def session_create(
        self, session_id: str, user_data: dict[str, Any], ttl: int = 86400,
    ) -> bool:
        """Create user session."""
        try:
            session_data = {
            ...
            return await self.cache_set(
                f"session:{session_id}", session_data, ttl, RedisNamespace.SESSION,
            )
    ...
    async def session_get(self, session_id: str) -> dict[str, Any] | None:
        ...
        try:
            session_data = await self.cache_get(
                f"session:{session_id}", RedisNamespace.SESSION,
            )
    ...
    async def websocket_subscribe(self, channel: str, callback: Callable) -> None:
