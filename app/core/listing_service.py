...
class FilterCriteria(BaseModel):
    ...
    value: Any | None = None
    values: list[Any] | None = None  # For IN, NOT_IN, BETWEEN operations
...
class ListingResponse(BaseModel):
    ...
    search_applied: SearchParams | None = None
...
class QueryBuilder:
    ...
    def apply_search(self, query: Select, search_params: SearchParams | None) -> Select:
        """Apply full-text search to query."""
        if not search_params or not search_params.query:
            return query

        search_query = search_params.query
        search_fields = search_params.fields or self.introspector.get_searchable_fields(
            self.model,
        )
    ...
    def apply_sorting(self, query: Select, sort_criteria: list[SortCriteria]) -> Select:
        ...
        if not sort_criteria:
            ...
            pk_columns = [
                col for col in self.model.__table__.columns if col.primary_key
            ]
            if pk_columns:
            ...
            if self.introspector.validate_field_access(self.model, relation_name):
                ...
                if field_info and field_info.get("type") == "relationship":
                    ...
                    if relationship.uselist:
                        ...
                        load_options.append(
                            joinedload(getattr(self.model, relation_name)),
                        )
...
class BaseListingService:
    ...
    async def list_items(
        self, session: AsyncSession, request: ListingRequest,
    ) -> ListingResponse:
                    ...
                    if rel_value is not None:
                        if isinstance(rel_value, list):
                            item_dict[relation_name] = [
                                self._serialize_related_item(rel_item)
                                for rel_item in rel_value
                            ]
                        else:
                            item_dict[relation_name] = self._serialize_related_item(
                                rel_value,
                            )
                    else:
...
def create_listing_params_model(model: type[Base]) -> type[BaseModel]:
    ...
    for field_name, field_info in model_fields.items():
        if field_info.get("type") != "relationship":
            ...
            filter_fields[f"{field_name}__eq"] = (
                Optional[str],
                Field(None, description=f"Filter {field_name} equals"),
            )
            filter_fields[f"{field_name}__ne"] = (
                Optional[str],
                Field(None, description=f"Filter {field_name} not equals"),
            )
            filter_fields[f"{field_name}__contains"] = (
                Optional[str],
                Field(None, description=f"Filter {field_name} contains"),
            )
            filter_fields[f"{field_name}__in"] = (
                Optional[list[str]],
                Field(None, description=f"Filter {field_name} in list"),
            )
    ...
    filter_fields.update(
        {
            ...
            "sort_order": (
                Optional[SortOrder],
                Field(SortOrder.ASC, description="Sort order"),
            ),
            "search": (Optional[str], Field(None, description="Search query")),
            "search_fields": (
                Optional[list[str]],
                Field(None, description="Fields to search in"),
            ),
        },
    )
...
def create_listing_route(model: type[Base], service: BaseListingService):
    ...
    async def list_route(session: AsyncSession, **params) -> ListingResponse:
...
def _convert_params_to_request(params: dict[str, Any]) -> ListingRequest:
    ...
    if params.get("search"):
        search = SearchParams(
            query=params["search"], fields=params.get("search_fields"),
        )
    ...
    if params.get("sort_by"):
        sort.append(
            SortCriteria(
                field=params["sort_by"], order=params.get("sort_order", SortOrder.ASC),
            ),
        )
    ...
    return ListingRequest(
        pagination=pagination, filters=filters, sort=sort, search=search,
    )
