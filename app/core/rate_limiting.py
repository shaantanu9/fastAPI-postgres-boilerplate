...
import json
import time
from typing import Dict, Optional, Callable, Any, Union, List, Type, cast
...
class EnhancedRateLimiter:
    ...
    def __init__(self, config: RateLimitConfig):
        ...
        self.fallback = InMemoryFallback(config.cleanup_interval) if config.enable_fallback else None
        ...
        for attempt in range(self.config.redis_retry_attempts):
            try:
                ...
                if attempt < self.config.redis_retry_attempts - 1:
                    await asyncio.sleep(self.config.redis_retry_delay * (2 ** attempt))  # Exponential backoff
        
        self._redis_healthy = False
        logger.error("Failed to connect to Redis after all attempts")
        
        if not self.config.enable_fallback:
            raise ConnectionError("Redis connection failed and fallback is disabled")
    ...
    async def _check_redis_rate_limit(self, key: str, limit: int, window: int) -> RateLimitResult:
        ...
        try:
            ...
            if current_count >= limit:
                ...
                return RateLimitResult(
                    ...
                    limit=limit,
                    remaining=0,
                    reset_time=int(oldest_time + window) if oldest_entries else int(current_time + window),
                    retry_after=retry_after
                )
...
def get_rate_limit_config() -> RateLimitConfig:
    ...
    try:
        ...
        return RateLimitConfig(
            ...
            enable_fallback=os.getenv('RATE_LIMIT_ENABLE_FALLBACK', 'true').lower() == 'true',
            ...
            enable_metrics=os.getenv('RATE_LIMIT_ENABLE_METRICS', 'true').lower() == 'true',
...
async def enhanced_rate_limit_exceeded_handler(request: Request, exc: RateLimitExceeded) -> JSONResponse:
    """
    ...
    try:
        ...
        logger.error(f"Error in enhanced rate limit handler: {str(e)}", exc_info=True)
...
async def setup_rate_limiting(app: FastAPI) -> None:
    ...
    try:
        # Get configuration
        config = get_rate_limit_config()
        logger.info(f"Setting up rate limiting with config: Redis={config.redis_url}, Fallback={config.enable_fallback}")
...
async def get_rate_limiter_dependency() -> Optional[EnhancedRateLimiter]:
