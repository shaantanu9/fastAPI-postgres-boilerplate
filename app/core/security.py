...
def require_role(role: str):
    def role_checker(user: User = Depends(get_current_active_user)):
        if not has_role(user, role):
            raise HTTPException(
                status_code=403, detail=f"User lacks required role: {role}",
            )
        return user
...
class EnterpriseSecurityService:
    ...
    async def handle_failed_login(self, db: Session, user, request: Request) -> None:
    ...
    async def handle_successful_login(self, db: Session, user, request: Request) -> None:
    ...
    def generate_mfa_qr_code(
        self, user_email: str, secret: str, app_name: str = "FastAPI App",
    ) -> str:
        """Generate QR code for MFA setup."""
        totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
            name=user_email, issuer_name=app_name,
        )
    ...
    async def log_security_event(
        self, db, user, event_type: str, category: str, data: dict, request,
    ) -> None:
