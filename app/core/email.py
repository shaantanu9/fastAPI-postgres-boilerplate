...
class EmailService:
    ...
    def verify_verification_token(
        self, token: str, user_id: str, email: str, max_age_hours: int = 24,
    ) -> bool:
        """Verify email verification token."""
        try:
            # Generate expected token
            str(
                int((datetime.utcnow() - timedelta(hours=max_age_hours)).timestamp()),
            )
            for hour_offset in range(max_age_hours + 1):
                ts = str(
                    int((datetime.utcnow() - timedelta(hours=hour_offset)).timestamp()),
                )
    ...
    def generate_password_reset_token(self, user_id: str, email: str) -> str:
        ...
        timestamp = str(int(datetime.utcnow().timestamp()))
        data = (
            f"{user_id}:{email}:{timestamp}:{random_part}:{settings.email_secret_key}"
        )
        return hashlib.sha256(data.encode()).hexdigest()

    async def send_verification_email(
        self, user_email: str, user_name: str, user_id: str,
    ) -> bool:
    ...
    async def send_invitation_email(
        ...
        try:
            ...
            subject = (
                f"You're invited to join {organization_name} on {settings.app_name}"
            )

            return await self.send_email(
                to_email=invitee_email, subject=subject, html_content=html_content,
            )

        except Exception as e:
            logger.error(f"Failed to send invitation email: {e!s}")
            return False
    ...
    async def send_welcome_email(self, user_email: str, user_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to send welcome email: {e!s}")
            return False
    ...
    async def send_password_changed_email(
        ...
        try:
            ...
            logger.error(f"Failed to send password changed email: {e!s}")
            return False
