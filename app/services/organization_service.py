...
class OrganizationService(BaseService):
    ...
    async def create_organization(
        self, db: AsyncSession, org_data: OrganizationCreate, owner_user_id: str,
    ) -> Organization:
        ...
        org_dict["trial_ends_at"] = datetime.utcnow() + timedelta(
            days=14,
        )  # 14-day trial
    ...
    async def get_organization_by_slug(
        ...
        query = (
            select(Organization)
            .where(and_(Organization.slug == slug, Organization.is_active))
            .options(
    ...
    async def update_organization(
        ...
        await self._check_organization_permission(
            db, organization_id, user_id, [MemberRole.OWNER, MemberRole.ADMIN],
        )
    ...
    async def delete_organization(
        ...
        await self._check_organization_permission(
            db, organization_id, user_id, [MemberRole.OWNER],
        )
        ...
        existing_invitation = await db.execute(
            select(OrganizationInvitation).where(
                and_(
                    ...
                    not OrganizationInvitation.is_accepted,
                    not OrganizationInvitation.is_expired,
    ...
    async def accept_organization_invitation(
        self, db: AsyncSession, token: str, user_id: str,
    ) -> OrganizationMembership:
        ...
        logger.info(
            f"User {user_id} accepted invitation to organization {invitation.organization_id}",
        )
        return membership
    ...
    async def update_member_role(
        ...
        # Check permissions
        await self._check_organization_permission(
            db, organization_id, updater_user_id, [MemberRole.OWNER, MemberRole.ADMIN],
        )
    ...
    async def remove_member(
        ...
        # Check permissions
        await self._check_organization_permission(
            db, organization_id, remover_user_id, [MemberRole.OWNER, MemberRole.ADMIN],
        )
        ...
        logger.info(
            f"Removed user {member_user_id} from organization {organization_id}",
        )
        return True
