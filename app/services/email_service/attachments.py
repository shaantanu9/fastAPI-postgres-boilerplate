...
class AttachmentHandler:
    ...
    def create_inline_image(
        image_path: str | Path, content_id: str, filename: str | None = None,
    ) -> EmailAttachment:
    ...
    async def add_attachment_to_message(
        msg: MIMEMultipart, attachment: EmailAttachment,
    ) -> None:
    ...
    def validate_attachments(
        ...
        for attachment in attachments:
            ...
            if allowed_types and attachment.content_type not in allowed_types:
                errors.append(
                    f"Attachment '{attachment.filename}' type '{attachment.content_type}' not allowed",
                )
            ...
            if any(
                ...
                errors.append(
                    f"Attachment '{attachment.filename}' has potentially dangerous extension",
                )

        # Check total size
        if total_size > max_size_mb:
            errors.append(
                f"Total attachment size {total_size:.2f}MB exceeds {max_size_mb}MB limit",
            )
    ...
    def create_pdf_attachment(
        ...
        return AttachmentHandler.create_attachment_from_bytes(
            content=content, filename=filename, content_type="application/pdf",
        )

    @staticmethod
    def create_excel_attachment(
        content: bytes, filename: str = "spreadsheet.xlsx",
    ) -> EmailAttachment:
    ...
    def create_csv_attachment(
        content: str | bytes, filename: str = "data.csv",
    ) -> EmailAttachment:
        ...
        return AttachmentHandler.create_attachment_from_bytes(
            content=content, filename=filename, content_type="text/csv",
        )
