...
class EmailScheduler:
    ...
    def __init__(self, procrastinate_app: Optional[procrastinate.App] = None):
        """
    ...
    async def schedule_bulk_emails(
        ...
        for email_request in email_requests:
            try:
                ...
                logger.error(f"Error scheduling bulk email {email_request.tracking_id}: {str(e)}")
    ...
    async def cancel_scheduled_email(self, job_id: str) -> bool:
        ...
        """
        if not self.procrastinate_app:
            raise ValueError("Procrastinate app not configured")
        ...
        try:
            ...
            logger.error(f"Error cancelling scheduled email job {job_id}: {str(e)}")
            return False
    ...
    async def get_scheduled_jobs(self) -> list[dict]:
        ...
        try:
            ...
            logger.error(f"Error fetching scheduled email jobs: {str(e)}")
            return []


def create_email_scheduler(procrastinate_app: Optional[procrastinate.App] = None) -> EmailScheduler:
    """
