...
@router.get(
    "/errors/dashboard", response_model=dict[str, Any], tags=["Error Dashboard"],
)
async def get_error_dashboard(
    hours: Annotated[int, Query(ge=1, le=168, description="Hours to look back (max 1 week)")] = 24,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve dashboard data: {e!s}",
        )


@router.get(
    "/errors/summary", response_model=ErrorSummaryResponse, tags=["Error Dashboard"],
)
async def get_error_summary(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve error summary: {e!s}",
        )


@router.get(
    "/errors/groups/{fingerprint}",
    response_model=ErrorDetailsResponse,
    tags=["Error Dashboard"],
)
async def get_error_group_details(
    fingerprint: str,
    limit: Annotated[int, Query(ge=1, le=500, description="Max events to return")] = 50,
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve error details: {e!s}",
        )


@router.get("/errors/alerts", tags=["Error Dashboard"])
async def get_error_alerts(
    limit: Annotated[int, Query(ge=1, le=500, description="Max alerts to return")] = 50,
    severity: Annotated[str | None, Query(regex="^(critical|warning|info)$", description="Filter by severity")] = None,
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve alerts: {e!s}")
...
async def get_error_stats():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve error statistics: {e!s}",
        )


@router.post("/errors/simulate", tags=["Error Dashboard"], include_in_schema=False)
async def simulate_error(
    error_type: Annotated[str, Query(description="Type of error to simulate")] = "test_error",
    message: Annotated[str, Query(description="Error message")] = "This is a test error",
    level: Annotated[str, Query(regex="^(ERROR|CRITICAL|WARNING)$", description="Error level")] = "ERROR",
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to simulate error: {e!s}")


@router.get(
    "/errors/dashboard/ui", response_class=HTMLResponse, include_in_schema=False,
)
...
async def error_system_health():
    ...
    Provides detailed health status information about the error aggregation system,
    including metrics on stored events, processing capacity, and recent activity.
    This endpoint is used by monitoring systems to verify the error tracking
    infrastructure is functioning correctly.

    Returns:
        Dict[str, Any]: Health status object containing:
            - status: Overall health status ('healthy', 'warning', or 'unhealthy')
            - timestamp: Current UTC timestamp
            - metrics: System metrics including event counts and configuration
            - last_event_time: Timestamp of the most recent error event
            - warning: Warning message if status is 'warning'
            - error: Error message if status is 'unhealthy'

    Raises:
        No exceptions are raised; errors are caught and returned as part of the
        response with an 'unhealthy' status.
    ...
    try:
        ...
        recent_events = [
            e
            for e in error_aggregator.events
        ...
        health_status = {
            ...
            "last_event_time": error_aggregator.events[-1].timestamp.isoformat() + "Z"
            if error_aggregator.events
            else None,
        }
