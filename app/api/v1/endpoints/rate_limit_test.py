...
async def test_rate_limit(request: Request):
    ...
    try:
        ...
        logger.error(f"Error in rate limit test endpoint: {str(e)}", exc_info=True)
        return JSONResponse(
...
async def test_no_rate_limit(request: Request):
    ...
    try:
        ...
        logger.error(f"Error in no-rate-limit test endpoint: {str(e)}", exc_info=True)
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={
                "detail": "An error occurred while processing your request.",
    ...
    return {
        ...
        "config": {
            "redis_url": config.redis_url.replace(
                ...
                "***:***@"
            ) if '@' in config.redis_url else config.redis_url,
...
async def reset_rate_limits(
    ...
    if not rate_limiter or not rate_limiter.redis_client:
        return {
            "message": "Cannot reset - Redis not available",
            "status": "error"
        }
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={
                "message": f"Error resetting rate limits: {str(e)}",
                "status": "error"
            }
        )
