...
from app.db.schemas.user import (
    ...
    PasswordResetRequest, BackupCodePasswordReset
)
...
async def register_user(
    ...
    db: AsyncSession = Depends(get_db),
    security_service: EnterpriseSecurityService = Depends(get_security_service)
):
    """
    ...
    """
    try:
        user = await enhanced_user_service.create_user(db, user_create, security_service)
...
async def verify_mfa(
    ...
    """
    try:
        is_valid = await enhanced_user_service.verify_mfa_setup(db, current_user.id, mfa_verify.token)
        return is_valid
    except HTTPException:
        raise
    except Exception as e:
...
async def check_password_strength(
    ...
    """
    strength = security_service.validate_password_strength(password)
    return strength
...
async def reset_password(
    ...
    """
    try:
        user = await enhanced_user_service.get_by_email(db, reset_password_request.email)
        if not user:
            raise HTTPException(
...
async def reset_password_with_backup_code(
    ...
    try:
        ...
        user = await enhanced_user_service.get_by_username_or_email(
            db, 
            username=request.username_or_email if "@" not in request.username_or_email else None,
            email=request.username_or_email if "@" in request.username_or_email else None
        )
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Backup code password reset failed: {str(e)}"
        )
...
async def get_security_events(
    ...
    """
    try:
        events = await enhanced_user_service.get_security_events(db, current_user.id)
        return events
    except HTTPException:
        raise
    except Exception as e:
