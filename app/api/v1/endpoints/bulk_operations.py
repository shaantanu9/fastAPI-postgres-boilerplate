...
from app.utils.task_queue import TaskPriority, enhanced_task_queue
...
class BulkCreateRequest(BaseModel):
    ...
    validate_parallel: bool | None = Field(
...
class BulkUpdateRequest(BaseModel):
    ...
    updates: list[dict[str, Any]] = Field(
        ..., description="List of update data with IDs",
    )
    batch_size: int | None = Field(
        50, description="Batch size for processing", ge=1, le=1000,
    )
...
class BulkDeleteRequest(BaseModel):
    ...
    batch_size: int | None = Field(
        100, description="Batch size for processing", ge=1, le=1000,
    )
...
class BulkSearchRequest(BaseModel):
    ...
    max_results_per_query: int | None = Field(100, description="Max results per query")
        ...
        if request.validate_parallel:
            ...
            for i, result in enumerate(validated_data):
                if isinstance(result, Exception):
                    validation_errors.append(f"Row {i + 1}: {result!s}")
                else:
            ...
            if validation_errors and not valid_data:
                raise HTTPException(
                    status_code=400,
                    detail={
                        "message": "All data failed validation",
...
async def bulk_update_users(
    ...
    try:
        ...
        updated_users = await user_service.bulk_update_users_parallel(
            db, request.updates,
        )
...
async def bulk_search_users(
    ...
    try:
        ...
        execution_time = time.time() - start_time
        sum(len(results) for results in search_results)
    ...
    try:
        ...
        def parse_csv_parallel(csv_content: str) -> list[dict[str, Any]]:
            ...
            if not all(col in reader.fieldnames for col in required_columns):
                msg = f"CSV must contain columns: {required_columns}"
                raise ValueError(msg)
        ...
        users_data = await execute_parallel(
            lambda: parse_csv_parallel(csv_data), [None], TaskType.CPU_BOUND,
        )
...
async def export_users_parallel(
    ...
    try:
        ...
        export_data = await user_service.export_users_parallel(db, filter_dict, format)
...
async def send_bulk_notifications(
    ...
    try:
        ...
        def send_notification(user_id: int) -> dict[str, Any]:
            ...
            logger.info(
                f"Sending {request.notification_type} to user {user_id}: {request.message}",
            )
            return {
                "user_id": user_id,
                "status": "sent",
        ...
        successful_sends = len(
            [r for r in notification_results if not isinstance(r, Exception)],
        )
...
async def get_queue_statistics():
    ...
    try:
        ...
        return {"queue_stats": stats, "health_check": health}
