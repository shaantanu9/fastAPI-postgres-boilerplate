...
async def test_quick_operation(delay: float = 1.0) -> dict[str, Any]:
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.exception(f"Quick operation failed: {e}")
        raise HTTPException(
            status_code=500,
            detail={
...
async def test_slow_operation(delay: float = 35.0) -> dict[str, Any]:
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.exception(f"Slow operation failed: {e}")
        raise HTTPException(
            status_code=500,
            detail={
                "error": "operation_failed",
...
async def test_heavy_computation(iterations: int = 5000000) -> dict[str, Any]:
    ...
    try:
        ...
        duration = time.time() - start_time
        logger.exception(f"Heavy computation failed: {e}")
        raise HTTPException(
            status_code=500,
            detail={
...
async def test_custom_timeout(
    delay: float = 8.0, timeout: float | None = 5.0,
) -> dict[str, Any]:
...
async def timeout_stress_test(
    concurrent_requests: int = 5, delay_per_request: float = 3.0,
) -> dict[str, Any]:
    ...
    async def single_operation(request_id: int):
        ...
        return {"request_id": request_id, "duration": round(time.time() - op_start, 3)}

    try:
        # Run concurrent operations
        tasks = [single_operation(i) for i in range(concurrent_requests)]
...
async def timeout_metrics(request: Request) -> dict[str, Any]:
    ...
    for middleware in request.app.middleware_stack:
        if (
            ...
            and middleware.cls.__name__ == "TimeoutMiddleware"
        ):
