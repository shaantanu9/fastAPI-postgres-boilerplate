...
from app.utils.task_scheduler import get_task_scheduler, TaskScheduler, TaskScheduleType
...
async def schedule_cron_task(
    ...
    priority_enum = getattr(ProcrastinateTaskPriority, priority, ProcrastinateTaskPriority.NORMAL)
...
async def get_procrastinate_jobs(
    ...
    try:
        jobs = await proc_manager.get_jobs(status=status, queue=queue, limit=limit)
        return jobs
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch Procrastinate jobs: {str(e)}"
        )
