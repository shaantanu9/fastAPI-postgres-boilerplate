...
class HealthChecker:
    ...
    async def check_redis(self) -> ComponentHealth:
        ...
        try:
            ...
            # Get operation
            result = await redis_manager.redis_client.get(test_key)
    ...
    async def check_external_services(self) -> ComponentHealth:
        ...
        try:
            ...
            unhealthy_services = [s for s in external_services if s["status"] == "unhealthy"]
            degraded_services = [s for s in external_services if s["status"] == "degraded"]
            ...
            return ComponentHealth(
                ...
                details={
                    ...
                    "healthy_services": len([s for s in external_services if s["status"] == "healthy"]),
...
async def get_metrics(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        metrics = {
            ...
            "system_cpu_percent": system_check.details.get("cpu_percent", 0) if system_check.details else 0,
            "system_memory_percent": system_check.details.get("memory_percent", 0) if system_check.details else 0,
            "system_disk_percent": system_check.details.get("disk_percent", 0) if system_check.details else 0,
            "redis_connected_clients": redis_check.details.get("connected_clients", 0) if redis_check.details else 0,
            "redis_memory_usage_percent": redis_check.details.get("memory_usage_percent", 0) if redis_check.details else 0,
            "database_response_time_ms": (await health_checker.check_database(db)).response_time_ms,
