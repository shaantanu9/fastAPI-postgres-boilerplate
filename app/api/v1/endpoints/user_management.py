...
async def send_verification_email(
    ...
    to the user's email address. The email contains instructions for completing the
    verification process. For security reasons, this endpoint does not reveal whether
    the email exists in the system.

    Args:
        request (EmailVerificationRequest): Request containing the user's email address.
        background_tasks (BackgroundTasks): FastAPI background task handler for async email sending.
        db (AsyncSession): Database session dependency.

    Returns:
        dict: Status message indicating the verification email was sent (if applicable).

    Raises:
        HTTPException: If the email is already verified.
...
async def verify_email(
    ...
    Validates the verification token against the user ID and email address.
    If valid, marks the user's email as verified in the database and sends
    a welcome email. This completes the user registration process and enables
    full account functionality.

    Args:
        token (str): The verification token sent to the user's email.
        user_id (str): The unique identifier of the user to verify.
        background_tasks (BackgroundTasks): FastAPI background task handler for async email sending.
        db (AsyncSession): Database session dependency.

    Returns:
        dict: Status message indicating if the email was successfully verified.

    Raises:
        HTTPException: If the user is not found or the token is invalid/expired.
...
async def reset_password(
    ...
    email_service.generate_password_reset_token(user.id, user.email)
...
async def invite_user(
    ...
    existing_user = await enhanced_user_service.get_by_username_or_email(
        db, email=invitation.email,
    )
    if existing_user:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_user_profile(current_user: Annotated[UserRead, Depends(get_current_user)]):
...
async def reactivate_account(email: str, db: Annotated[AsyncSession, Depends(get_db)]):
