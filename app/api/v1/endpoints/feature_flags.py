...
from app.core.dependencies import get_current_tenant, get_current_user, get_current_admin_user
from app.utils.feature_flags import FeatureFlagManager, Feature, FeatureState, FeatureContext
...
async def get_available_features(
    ...
    subscription_tier: Optional[str] = Query(None, description="Subscription tier to check features for"),
...
@router.post("/admin/features", response_model=Feature, status_code=status.HTTP_201_CREATED)
...
@router.post("/admin/tenants/{tenant_id}/overrides", status_code=status.HTTP_201_CREATED)
async def create_tenant_override(
    tenant_id: str = Path(..., description="Tenant ID to create override for"),
    override: FeatureOverrideRequest = Body(...),
    feature_manager: FeatureFlagManager = Depends(get_feature_manager),
    _: Dict = Depends(get_current_admin_user)  # Ensure admin access
):
    """
