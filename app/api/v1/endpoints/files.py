...
async def upload_file(
    ...
    storage_type: Annotated[FileStorageType | None, Form()] = None,
    tags: Annotated[str | None, Form()] = None,  # Comma-separated tags
    ...
    try:
        return await file_manager.upload_file(
            file=file, storage_type=storage_type, tags=parsed_tags,
        )
...
async def generate_presigned_upload_url(
    ...
    expires_in: Annotated[int, Query(description="URL expiration in seconds")] = 3600,
    file_size_limit: Annotated[int | None, Query(description="File size limit in bytes")] = None,
    file_manager: FileManager = Depends(get_file_manager),
) -> dict[str, Any]:
    ...
    """
    try:
        return await file_manager.generate_presigned_upload_url(
            filename=filename, expires_in=expires_in, file_size_limit=file_size_limit,
        )
    except Exception as e:
        raise HTTPException(
...
async def generate_presigned_download_url(
    file_id: str,
    expires_in: Annotated[int, Query(description="URL expiration in seconds")] = 3600,
    filename: Annotated[str | None, Query(description="Download filename")] = None,
    file_manager: FileManager = Depends(get_file_manager),
) -> dict[str, str]:
...
async def get_file_info(
    file_id: str, file_manager: Annotated[FileManager, Depends(get_file_manager)],
) -> FileMetadata:
...
async def list_files(
    category: Annotated[FileCategory | None, Query(description="Filter by file category")] = None,
    ...
    offset: Annotated[int, Query(ge=0, description="Number of files to skip")] = 0,
    file_manager: FileManager = Depends(get_file_manager),
) -> list[FileMetadata]:
    ...
    return await file_manager.list_files(
        category=category, tags=parsed_tags, limit=limit, offset=offset,
    )
