...
from fastapi import APIRouter, Depends, Request, Query as QueryParam, HTTPException
...
from app.utils.pagination import (
    PaginationParams, CursorPaginationParams, TimePaginationParams,
    get_pagination_params, get_cursor_pagination_params, get_time_pagination_params,
    PaginatedResponse, CursorPaginatedResponse, TimePaginatedResponse,
    PaginationBuilder, build_pagination_links, build_cursor_links
)
from app.core.versioning import (
    get_api_version, validate_api_version, add_version_headers,
    versioned_route, VersioningStrategy
)
...
async def get_users_paginated(
    ...
    links = build_pagination_links(request, pagination_info)
...
async def get_users_cursor(
    ...
    # Build cursor links
    links = build_cursor_links(request, cursor_info)
...
async def get_large_dataset(
    size: int = QueryParam(1000, ge=100, le=10000, description="Number of items to generate")
):
    """
...
async def versioned_endpoint(request: Request):
    ...
    elif version.major == 1 and version.minor >= 1:
        return {
...
async def performance_test(
    delay: float = QueryParam(0.1, ge=0, le=5.0, description="Artificial delay in seconds")
):
    """
...
async def compression_test(
    content_type: str = QueryParam(
        "json",
        description="Type of content to generate: json, text, random, structured"
    ),
    size_kb: int = QueryParam(100, ge=1, le=1000, description="Approximate size in KB")
):
    """
...
async def feature_status():
    ...
    return {
        ...
        "status": "All advanced features are available!" if all([
            middleware_available, versioning_available, pagination_available
