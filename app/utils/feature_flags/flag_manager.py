...
from .models import Feature, FeatureOverride, FeatureState, FeatureContext, FeatureScope
...
class FeatureFlagManager:
        ...
        if self.redis_client:
            try:
                await self.redis_client.set(
                    f"feature:{feature.key}",
                    feature.json(),
                    ex=self.cache_ttl
                )
            except Exception as e:
                logger.error(f"Failed to cache feature in Redis: {e}")
    ...
    async def get_feature(self, key: str) -> Optional[Feature]:
        ...
        if self.redis_client:
            try:
                ...
                logger.error(f"Failed to get feature from Redis: {e}")
    ...
    def _is_in_percentage_rollout(self, feature: Feature, context: FeatureContext) -> bool:
        """
