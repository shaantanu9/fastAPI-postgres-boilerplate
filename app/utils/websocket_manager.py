...
class RedisPubSubManager:
    ...
    async def disconnect(self) -> None:
        ...
        for channel, pubsub in self._channel_subscriptions.items():
            await pubsub.close()
...
class WebSocketManager:
    ...
    async def connect(
        ...
        logger.info(f"Client connected to tenant:{tenant_id}, room:{room_id}, conn_id:{connection.connection_id}")
    ...
    async def disconnect(
        ...
        if connection:
            ...
            logger.warning(f"Attempted to disconnect a non-existing connection: tenant:{tenant_id}, room:{room_id}")
    ...
    async def send_to_connection(
        ...
        if not connection:
            logger.warning(f"Attempted to send to non-existing connection: {connection_id}")
            return False
    ...
    async def get_connections_count(self, tenant_id: Optional[str] = None) -> Dict[str, int]:
        """
    ...
    async def _process_messages(
        ...
        try:
            ...
            logger.info(f"Message handler for tenant:{tenant_id}, room:{room_id} was cancelled")
        except Exception as e:
    ...
    async def _deliver_message_to_room(
        ...
        try:
            ...
            if isinstance(message_data, dict) and "exclude_connection_id" in message_data:
