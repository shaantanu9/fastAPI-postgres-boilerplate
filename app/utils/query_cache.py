...
class QueryComplexity:
    ...
    @staticmethod
    def calculate_complexity(query: Union[Select, str]) -> int:
        """
        ...
        if isinstance(query, str):
            ...
            subquery_count = query_lower.count('select', 1)  # Start at index 1 to skip the first SELECT
...
class QueryCacheManager:
    ...
    def _generate_cache_key(self, query: Union[Select, str], params: Dict = None) -> str:
    ...
    async def set(
        ...
        data: Dict, 
        ttl: int = 300,
        model_dependencies: List[Type[DeclarativeMeta]] = None
    ) -> bool:
        """
    ...
    def get_dynamic_ttl(self, query: Union[Select, str], base_ttl: int = 300) -> int:
        """
        ...
        if complexity <= 10:  # Simple queries
            return base_ttl
        elif complexity <= 20:  # Moderate queries
            return base_ttl * 2
        elif complexity <= 30:  # Complex queries
            return base_ttl * 3
        else:  # Very complex queries
            return base_ttl * 5
    ...
    def get_metrics(self) -> Dict:
        ...
        return {
            **self.metrics,
            ...
            "models_last_updated": {k: v.isoformat() for k, v in self.models_last_updated.items()},
        }
...
async def init_query_cache(redis_url: str = None):
...
class cached_query:
    ...
    def __init__(
        self, 
        ttl: int = 300, 
        models: List[Type[DeclarativeMeta]] = None,
        ...
        self.condition = condition  # Optional function that returns True if result should be cached
    ...
    def __call__(self, func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            for param_name, param_value in bound_args.arguments.items():
                if param_name != 'db' and param_name != 'self':  # Skip db session
            ...
            # Cache the result
            await query_cache_manager.set(key, result, ttl=ttl, model_dependencies=self.models)
            logger.debug(f"Cached {func_name} (took {execution_time:.4f}s), TTL: {ttl}s")
