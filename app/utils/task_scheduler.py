...
class ScheduleMetadata:
    ...
    def _calculate_next_run(self) -> datetime | None:
        ...
        if self.schedule_type == TaskScheduleType.INTERVAL:
        ...
        if self.schedule_type == TaskScheduleType.CRON:
            cron_expr = self.schedule_config.get(
                "cron_expr", "0 0 * * *",
...
class TaskScheduler:
    ...
    async def _execute_scheduled_task(self, job_id: str, metadata: ScheduleMetadata) -> None:
        ...
        try:
            ...
            try:
                if hasattr(procrastinate_manager, f"defer_{task_name}"):
                    ...
                    proc_job_id = await procrastinate_manager.app.configure_task(
                        name=task_name,
                    ).defer(**task_kwargs)
                    result = {"procrastinate_job_id": proc_job_id}
    ...
    async def schedule_task(
        ...
        config["task_info"] = {"task_name": task_name, "task_kwargs": task_kwargs or {}}
    ...
    def get_scheduled_tasks(self, tags: list[str] | None = None) -> list[dict[str, Any]]:
    ...
    def get_task_stats(self) -> dict[str, Any]:
        ...
        for status in TaskStatus:
            status_counts[status.value] = sum(
                1 for task in task_history if task["status"] == status.value
            )
...
async def register_common_tasks() -> None:
