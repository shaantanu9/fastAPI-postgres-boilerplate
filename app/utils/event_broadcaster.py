...
from app.utils.websocket_manager import get_websocket_manager, RedisPubSubManager
...
class EventPayload(BaseModel):
    ...
    def validate_event_type(cls, v):
        ...
        if not v or "." not in v:
            raise ValueError("event_type must follow format 'namespace.action'")
        return v
...
class EventBroadcaster:
    ...
    async def subscribe(
        ...
        for event_type in event_types:
            ...
            if channel != "events:system" and channel != "events:*:*":
                pubsub = await self.pubsub_manager.subscribe(channel)
    ...
    async def get_recent_events(
        ...
        if event_types:
            ...
            events = [
                e for e in events 
                if any(self._event_type_matches(e.event_type, pattern) for pattern in event_types)
            ]
    ...
    def _add_to_recent_events(self, tenant_id: str, event: EventPayload) -> None:
        ...
        if len(self.recent_events[tenant_id]) > self.max_recent_events:
            ...
            self.recent_events[tenant_id] = self.recent_events[tenant_id][-self.max_recent_events:]
