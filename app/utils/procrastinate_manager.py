...
async def process_bulk_data(
    data_items: list[dict[str, Any]], task_type: str = "parallel",
) -> dict[str, Any]:
        ...
        if task_type == "parallel":
            ...
            try:
                results = await asyncio.gather(
                    *[process_item(item) for item in data_items], return_exceptions=True,
                )
...
async def send_notification(
    notification_type: str, recipient: str, message: str, **kwargs,
) -> dict[str, Any]:
...
async def process_file(
    file_path: str, operation: str = "analyze", **kwargs,
) -> dict[str, Any]:
...
async def generate_analytics_report(
    report_type: str, date_range: dict[str, str], **kwargs,
) -> dict[str, Any]:
...
async def cleanup_old_data(
    days_old: int = 30, scheduled_for: str | None = None,
) -> dict[str, Any]:
...
async def system_health_check(scheduled_for: str | None = None) -> dict[str, Any]:
...
class ProcrastinateManager:
    ...
    def _initialize_sync(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Procrastinate initialization failed, tasks will be disabled",
            )
    ...
    async def defer_user_processing(
        self, user_id: int, operation: str, **kwargs,
    ) -> str:
        ...
        async with self.app.open_async():
            job_id = await process_user_data.defer_async(
                user_id=user_id, operation=operation, **kwargs,
            )
            logger.info(f"Deferred user processing task: {job_id}")
            return str(job_id)

    async def defer_bulk_processing(
        self, data_items: list[dict[str, Any]], task_type: str = "parallel",
    ) -> str:
        ...
        async with self.app.open_async():
            job_id = await process_bulk_data.defer_async(
                data_items=data_items, task_type=task_type,
            )
            logger.info(f"Deferred bulk processing task: {job_id}")
            return str(job_id)

    async def defer_notification(
        self, notification_type: str, recipient: str, message: str, **kwargs,
    ) -> str:
    ...
    async def defer_file_processing(
        self, file_path: str, operation: str = "analyze", **kwargs,
    ) -> str:
        ...
        async with self.app.open_async():
            job_id = await process_file.defer_async(
                file_path=file_path, operation=operation, **kwargs,
            )
            logger.info(f"Deferred file processing task: {job_id}")
            return str(job_id)

    async def defer_analytics_report(
        self, report_type: str, date_range: dict[str, str], **kwargs,
    ) -> str:
        ...
        async with self.app.open_async():
            job_id = await generate_analytics_report.defer_async(
                report_type=report_type, date_range=date_range, **kwargs,
            )
            logger.info(f"Deferred analytics report task: {job_id}")
            return str(job_id)

    async def schedule_cleanup(self, days_old: int = 30, at: datetime | None = None) -> str:
        ...
        if at is None:
            at = datetime.now(UTC) + timedelta(hours=1)  # Default to 1 hour from now
        ...
        async with self.app.open_async():
            ...
            delay_seconds = max(delay_seconds, 0)  # Execute immediately if scheduled in the past

            job_id = await cleanup_old_data.configure(
    ...
    async def schedule_health_check(self, at: datetime | None = None) -> str:
        ...
        async with self.app.open_async():
            ...
            job_id = await system_health_check.configure(
                schedule_in={"seconds": delay_seconds},
            ).defer_async(
                scheduled_for=at.isoformat(),  # Keep as metadata
            )
    ...
    async def purge_queue(
        self, queue_name: str, status_filter: str | None = None,
    ) -> dict[str, Any]:
            ...
            async for session in get_db():
                ...
                if status_filter:
                    ...
                    result = await session.execute(
                        query, {"queue_name": queue_name, "status": status_filter},
                    )
                else:
                    query = text(
    ...
    async def retry_failed_jobs(
        ...
        try:
            ...
            async for session in get_db():
                ...
                result = await session.execute(
                    query, {"queue_name": queue_name, "limit": limit},
                )
                await session.commit()

                retried_count = result.rowcount
                logger.info(
                    f"Retried {retried_count} failed jobs in queue {queue_name}",
                )
        ...
        try:
            ...
            async for session in get_db():
                ...
                events_result = await session.execute(
                    events_query, {"job_id": int(job_id)},
                )
                events = []
                ...
                return {
                    "job": {
                        ...
                        "args": job_row.args,
                        "scheduled_at": job_row.scheduled_at.isoformat()
                        if job_row.scheduled_at
                        else None,
    ...
    async def get_registered_tasks(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "queues": list({task["queue"] for task in tasks.values()}),
            }
            ...
            async for session in get_db():
                ...
                for worker_row in workers_result:
                    workers.append(
                        {
                            "id": worker_row.id,
                            "last_heartbeat": worker_row.last_heartbeat.isoformat()
                            if worker_row.last_heartbeat
                            else None,
                            "status": "active"
                            if worker_row.last_heartbeat
                            and (
                                datetime.now(UTC)
                                - worker_row.last_heartbeat.replace(tzinfo=UTC)
                            ).total_seconds()
                            < 60
                            else "inactive",
                        },
                ...
                worker_job_counts = {
                    row.worker_id: row.job_count for row in active_jobs_result
                }
    ...
    async def cleanup_old_jobs(
        self, days_old: int = 30, status_filter: str | None = None,
    ) -> dict[str, Any]:
        ...
        try:
            ...
            async for session in get_db():
                ...
                if status_filter:
                    ...
                    """)
                    result = await session.execute(
                        query, {"days": days_old, "status": status_filter},
                    )
                else:
                    query = text("""
                        DELETE FROM procrastinate_jobs
                ...
                logger.info(
                    f"Cleaned up {cleaned_count} old jobs (older than {days_old} days)",
                )
...
def register_plugin_blueprint(blueprint, namespace: str | None = None) -> None:
