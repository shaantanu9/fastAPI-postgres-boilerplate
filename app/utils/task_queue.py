...
from app.utils.concurrent_utils import execute_parallel, TaskType, concurrent_manager
...
class EnhancedAsyncTaskQueue:
    ...
    async def worker(self, worker_id: int):
        ...
        while self.running:
            try:
                ...
                logger.info(f"Worker {worker_id} executing task: {task.func.__name__} (ID: {task.id})")
                ...
                try:
                    ...
                    if task.retry_count < task.max_retries:
                        # Retry the task
                        task.retry_count += 1
                        retry_delay = min(2 ** task.retry_count, 60)  # Exponential backoff
                        
                        logger.info(f"Retrying task {task.id} in {retry_delay}s (attempt {task.retry_count}/{task.max_retries})")
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'success_rate': (self.processed_tasks / max(self.processed_tasks + self.failed_tasks, 1)) * 100
        }
...
class AsyncTaskQueue(EnhancedAsyncTaskQueue):
    ...
    def __init__(self):
        super().__init__(max_workers=5)  # Conservative default for backward compatibility
